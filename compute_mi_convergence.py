...
for mode in ['data', 'baseline', 'permutation']:
    ...
    if mode == 'permutation':
        ...
        model.update_model_params_from_conll_file(paths, -1, w2c, mode)
        if mode == 'data':
    ...
    print(mode, 'MI(w1;w2):\t\t\t', word_mi)
    print(mode, 'MI(pos1;pos2)\t\t\t', pos_mi)
    print(mode, 'MI(lex1;lex2)\t\t\t', lex_mi)

    word_pairs_tmp = model.data

    for k in [0.01, 0.02, 0.05, 0.08, 0.1, 0.2, 0.4, 0.5, 0.8, 1.0]:
        small_model = InfoModel()
        cut = int(len(word_pairs_tmp)*k)
        small_model.update_model_params_from_word_pairs(word_pairs_tmp[:cut], w2c, permuted=False)

        print(mode, 'Sample size:', np.sum(list(small_model.fdist_w1.values())), small_model.num_of_datapoints)
        word_mi = small_model.get_word_mutual_info()
        pos_mi = small_model.get_pos_mutual_info()
        lex_mi = small_model.get_lex_mutual_info()
        print(mode, 'MI(w1;w2):\t\t\t', word_mi)
        print(mode, 'MI(pos1;pos2)\t\t\t', pos_mi)
        print(mode, 'MI(lex1;lex2)\t\t\t', lex_mi)
